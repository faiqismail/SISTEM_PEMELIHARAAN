...
include "../inc/auth.php";          // ✅ Tambah ini

checkAuth('admin');                 // ✅ Ganti requireAuth('admin')

$id_login = $_SESSION['id_login'];
$username = $_SESSION['username'];
$id_user  = $_SESSION['id_user'];

// ✅ Hapus query ulang ke DB untuk ambil username & role
// Karena sudah ada di $_SESSION, tidak perlu query lagi
                ...
                <div class="flex items-center gap-3">
                    ...
                    <?php if($bidang != ''): ?>
                        <span class="px-3 py-1 bg-white/20 rounded-full text-xs font-semibold">
                            <i class="fas fa-filter mr-1"></i>Bidang: <?= htmlspecialchars($bidang) ?>
                        </span>
                    <?php endif; ?>
                </div>
                <div class="flex items-center gap-4">
                    <form method="GET" action="" class="flex items-center gap-2">
                        <select name="bidang" class="text-xs px-2 py-1 border border-white/30 rounded bg-white/10 text-white" onchange="this.form.submit()" style="color: white;">
                            <option value="" <?= $bidang == '' ? 'selected' : '' ?> style="color: black;">Semua Bidang</option>
                            <?php 
                            mysqli_data_seek($result_bidang, 0);
                            while ($row_bidang = mysqli_fetch_assoc($result_bidang)): ?>
                                <option value="<?= htmlspecialchars($row_bidang['bidang']) ?>" <?= $bidang == $row_bidang['bidang'] ? 'selected' : '' ?> style="color: black;">
