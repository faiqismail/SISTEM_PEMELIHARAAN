...
if (isset($_POST['kembalikan_sa'])) {
    ...
    // Update status ke Dikembalikan_sa dengan catatan_qc
...
if (isset($_POST['minta_persetujuan_pengawas'])) {
    ...
    // Update status ke Minta_Persetujuan_Pengawas dan set persetujuan_pengawas = Menunggu
    $update = mysqli_query($connection, "
        UPDATE permintaan_perbaikan 
...
if ($filter_status != '') {
    if ($filter_status == 'Menunggu_Pengawas') {
        $where = "WHERE p.status = 'Minta_Persetujuan_Pengawas' AND p.persetujuan_pengawas = 'Menunggu'";
        if ($search != '') {
            $where .= " AND (k.nopol LIKE '%$search%' OR k.jenis_kendaraan LIKE '%$search%' OR k.bidang LIKE '%$search%' OR p.nomor_pengajuan LIKE '%$search%')";
        }
    } else {
...
$count_disetujui_karu = mysqli_num_rows(mysqli_query($connection, "SELECT * FROM permintaan_perbaikan WHERE status='Disetujui_KARU_QC'"));
$count_menunggu_pengawas = mysqli_num_rows(mysqli_query($connection, "SELECT * FROM permintaan_perbaikan WHERE status='Minta_Persetujuan_Pengawas' AND persetujuan_pengawas='Menunggu'"));
