...
===================================================================== */
function buildJasaWhere($connection) {
    ...
    if ($search !== '') {
        ...
        $where_conditions[] = "(kode_pekerjaan LIKE '%$search_esc%' OR nama_pekerjaan LIKE '%$search_esc%')";
    }
...
/* =====================================================================
   HELPER: cek apakah sebuah jasa SUDAH PERNAH dipakai di permintaan
...
===================================================================== */
function jasaJumlahPemakaian($connection, $id_jasa) {
    $id_jasa = mysqli_real_escape_string($connection, $id_jasa);
    $q = mysqli_query($connection, "SELECT COUNT(*) AS total FROM perbaikan_detail WHERE id_jasa='$id_jasa'");
...
if (isset($_GET['template']) && $_GET['template'] == '1') {
    ...
    $dataRows = [
        ['kode', 'nama', 'harga', 'status'],
        ['SV001', 'SERVICE BERKALA', '150000', 'Y'],
    ];

    native_csv_output($dataRows, 'template_import_jasa.csv');
}
...
if (isset($_GET['export']) && $_GET['export'] == '1') {
    ...
    [$exp_where, , ] = buildJasaWhere($connection);

    $exp_q = mysqli_query($connection, "
        SELECT mj.*, (SELECT COUNT(*) FROM perbaikan_detail pd WHERE pd.id_jasa = mj.id_jasa) AS jumlah_pemakaian
        FROM master_jasa mj $exp_where
        ORDER BY mj.aktif DESC, mj.kode_pekerjaan ASC
    ");

    $dataRows = [['No', 'Kode', 'Nama Jasa', 'Harga', 'Status', 'Pernah Dipakai', 'Tanggal Dibuat']];
    ...
    native_xlsx_output([
        ['name' => 'Data Jasa', 'rows' => $dataRows, 'headerRows' => 1, 'widths' => [5, 6, 14, 35, 14, 12, 14, 18]],
    ], 'export_jasa_' . date('Ymd_His') . '.xls');
}

/* =====================================================================
   FITUR: IMPORT DATA (CSV) - PHP NATIVE, TANPA LIBRARY
   Format kolom WAJIB: kode | nama | harga | status
   ...
   - Setelah struktur file valid, tiap baris data divalidasi lagi: kode &
     nama wajib diisi, harga wajib angka > 0, dan kode yang berstatus
     Aktif tidak boleh bentrok dengan kode Aktif lain (baik yang sudah
     ada di database maupun sesama baris di file yang sama). Baris yang
     tidak lolos DILEWATI (bukan menggagalkan seluruh file) dan
     dilaporkan jelas baris ke berapa saja.
===================================================================== */
if (isset($_POST['import_submit'])) {
    ...
    $expectedHeader = ['kode', 'nama', 'harga', 'status'];
    ...
    foreach ($rows as $row) {
        ...
        $kode_raw   = isset($row[0]) ? strtoupper(trim((string) $row[0])) : '';
         $nama_raw   = isset($row[1]) ? trim((string) $row[1]) : '';
        $harga_raw  = isset($row[2]) ? trim((string) $row[2]) : '';
        $status_raw = isset($row[3]) ? trim((string) $row[3]) : '';

        // Lewati baris kosong total
        if ($kode_raw === '' && $nama_raw === '' && $harga_raw === '') continue;

        if ($kode_raw === '' || $nama_raw === '') {
            $dilewati++;
        ...
        if ($status_match === 'Y') {
            ...
            $cek_aktif = mysqli_query($connection, "SELECT id_jasa FROM master_jasa WHERE kode_pekerjaan='$kode_esc_check' AND aktif='Y'");
        ...
        $kode_esc   = mysqli_real_escape_string($connection, strtoupper($kode_raw));
        ...
        $ok = mysqli_query($connection, "INSERT INTO master_jasa
            (kode_pekerjaan, nama_pekerjaan, harga, aktif)
            VALUES ('$kode_esc','$nama_esc','$harga','$status_esc')");
    ...
    if ($dilewati > 0) {
        $pesan .= "\\nDilewati: $dilewati data (baris ke: " . implode(', ', $baris_dilewati) . ")\\nPenyebab umum: kode/nama kosong, harga tidak valid, kode bentrok dengan jasa lain yang masih Aktif, atau status tidak dikenali.";
    }

    echo "<script>
        alert('" . addslashes($pesan) . "');
        window.location.href='master_jasa.php?success=import&count=$berhasil&skip=$dilewati';
    </script>";
    exit;
}
...
if (isset($_POST['bulk_action']) && isset($_POST['selected_ids']) && is_array($_POST['selected_ids'])) {
    ...
    $redirect  = 'master_jasa.php' . ($backQuery ? '?' . $backQuery : '');
    ...
    if ($action === 'nonaktif') {
        ...
        mysqli_query($connection, "UPDATE master_jasa SET aktif='N' WHERE id_jasa IN ($idListSql)");
        $jumlah = count($ids);

        echo "<script>
            alert('✅ $jumlah data jasa berhasil dinonaktifkan!');
        ...
        // Setiap jasa yang mau diaktifkan tetap dicek satu-satu: kodenya
        // tidak boleh bentrok dengan jasa lain yang masih Aktif (aturan
        // sama seperti aktifkan satuan).
        $berhasil_aktif = 0;
        $gagal_bentrok  = 0;

        foreach ($ids as $id) {
            $r = mysqli_fetch_assoc(mysqli_query($connection, "SELECT kode_pekerjaan FROM master_jasa WHERE id_jasa='$id'"));
            if (!$r) continue;

            $kode_esc = mysqli_real_escape_string($connection, $r['kode_pekerjaan']);
            $cek = mysqli_query($connection, "
                SELECT id_jasa FROM master_jasa
                WHERE kode_pekerjaan='$kode_esc' AND id_jasa != '$id' AND aktif='Y'
            ");
            ...
            mysqli_query($connection, "UPDATE master_jasa SET aktif='Y' WHERE id_jasa='$id'");
            $berhasil_aktif++;
        }

        $pesan = "✅ $berhasil_aktif data jasa berhasil diaktifkan kembali!";
        if ($gagal_bentrok > 0) {
            $pesan .= "\\n⚠️ $gagal_bentrok data TIDAK bisa diaktifkan karena kodenya masih dipakai jasa lain yang sedang Aktif.";
        }
        ...
        // Jasa yang sudah pernah dipakai di permintaan perbaikan (termasuk
        // riwayat) otomatis dilewati -> tidak bisa dihapus, hanya bisa
        // dinonaktifkan.
        $berhasil_hapus = 0;
        $gagal_relasi   = 0;

        foreach ($ids as $id) {
            if (jasaJumlahPemakaian($connection, $id) > 0) {
                $gagal_relasi++;
                continue;
            }
            mysqli_query($connection, "DELETE FROM master_jasa WHERE id_jasa='$id'");
            $berhasil_hapus++;
        }

        $pesan = "✅ $berhasil_hapus data jasa berhasil dihapus!";
        if ($gagal_relasi > 0) {
...
if (isset($_POST['simpan'])) {
    ...
    // ===== KUNCI HARGA UNTUK JASA YANG SUDAH PERNAH DIPAKAI =====
    // Ini pengecekan sisi server (bukan cuma sisi tampilan), supaya harga
    // tidak bisa diubah sekalipun orang mengakali lewat DevTools/form
    // yang dimodifikasi. Kalau jasa ini sudah pernah masuk ke permintaan
    // perbaikan manapun (termasuk yang sudah Selesai / riwayat lama),
    // harga yang dipakai TETAP harga lama di database, apapun yang
    // dikirim dari form.
    $harga = $harga_input;
    if ($id_jasa != '' && jasaJumlahPemakaian($connection, $id_jasa) > 0) {
        $row_lama = mysqli_fetch_assoc(mysqli_query($connection, "SELECT harga FROM master_jasa WHERE id_jasa='$id_jasa'"));
        if ($row_lama) {
...
if (isset($_GET['edit'])) {
    ...
    $q       = mysqli_query($connection, "
        SELECT mj.*, (SELECT COUNT(*) FROM perbaikan_detail pd WHERE pd.id_jasa = mj.id_jasa) AS jumlah_pemakaian
        FROM master_jasa mj WHERE mj.id_jasa='$id_edit'
    ");
...
===================== */
[$where, $search, $filter_status] = buildJasaWhere($connection);
...
function currentJasaFilterQuery() {
    $params = [];
...
$filterQueryString = currentJasaFilterQuery();
?>
<!DOCTYPE html>
<html lang="id">
<head>
                ...
                <div class="bg-yellow-50 border-2 border-yellow-300 rounded-lg p-3 mb-4">
                    <div class="flex items-start gap-2">
                        ...
                        <p class="text-xs text-yellow-800">
                            <strong>Jasa ini sudah pernah dipakai</strong> di <strong><?= (int) $edit['jumlah_pemakaian'] ?> permintaan perbaikan</strong> (termasuk yang sudah selesai / riwayat lama).
                            Untuk mencegah selisih perhitungan biaya di kemudian hari, <strong>harga tidak bisa diubah lagi</strong>.
                            Kalau butuh harga baru: nonaktifkan jasa ini, lalu <strong>buat data jasa baru</strong> dengan harga terkini.
                        </p>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <form method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-yellow-700 mt-1">
                            ...
                            Harga dikunci karena jasa ini sudah pernah dipakai di riwayat permintaan.
                        </p>
                        <?php else: ?>
                        <p class="text-xs text-gray-500 mt-1">
                            <i class="fas fa-info-circle mr-1"></i>
        ...
        <div class="xl:col-span-7 2xl:col-span-8">
            <div class="card rounded-2xl shadow-2xl p-4 sm:p-6">
                ...
                <div class="toolbar-import-export">
                    <a href="master_jasa.php?template=1" class="btn-sm btn-secondary-sm">
                    ...
                    <a href="master_jasa.php?export=1<?= $filterQueryString ? '&' . htmlspecialchars($filterQueryString) : '' ?>" class="btn-sm btn-excel-sm">
                ...
                <div class="note-box" style="padding: 0;">
                    ...
                    <div id="noteImportContent" style="display:none; padding: 0 16px 14px 16px;">
                        <ul>
                            ...
                            <li>Header baris pertama WAJIB persis: <strong>kode, nama, harga, status</strong> — kalau strukturnya berubah, import akan <strong>GAGAL TOTAL</strong> (tidak ada data yang dipaksa masuk).</li>
                            <li>Kolom <strong>kode</strong> & <strong>nama</strong> wajib diisi, kolom <strong>harga</strong> wajib angka lebih dari 0.</li>
                            <li>Kolom <strong>status</strong> isi <strong>Y</strong> (Aktif) atau <strong>N</strong> (Tidak Aktif), kosong = otomatis Aktif.</li>
                            <li>Kode yang berstatus Aktif tidak boleh sama dengan kode jasa lain yang masih Aktif — baris yang bentrok otomatis dilewati.</li>
                ...
                <form method="POST" id="bulkForm">
                    ...
                    <div class="scroll-container rounded-lg border-2 border-gray-200 overflow-x-auto">
                        <table class="w-full table-fixed min-w-[1050px]">
                            <thead class="text-white sticky top-0" style="background:rgb(9, 120, 83);">
                                <tr>
                                    ...
                                    <th class="px-3 py-4 text-left font-semibold w-[29%]">Nama Jasa</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
                                ...
                                $q  = mysqli_query(
                                    $connection,
                                    "SELECT mj.*, (SELECT COUNT(*) FROM perbaikan_detail pd WHERE pd.id_jasa = mj.id_jasa) AS jumlah_pemakaian
                                     FROM master_jasa mj $where ORDER BY mj.aktif DESC, mj.kode_pekerjaan ASC LIMIT $per_page OFFSET $offset"
                                );
                                $no = 0;
                                ...
                                <tr class="table-row <?= $d['aktif'] == 'N' ? 'opacity-60 bg-gray-100' : '' ?>" data-status="<?= $d['aktif'] ?>">
                                    <td class="px-2 py-4 checkbox-col">
                                        <input type="checkbox" class="row-checkbox" name="selected_ids[]"
                                               value="<?= $d['id_jasa'] ?>" onclick="onRowCheck(this)">
                                    </td>
                                    ...
                                    <td class="px-3 py-4 text-gray-800 w-[29%]">
                ...
                <div class="mt-4 space-y-2">
                    ...
                    <p class="text-sm text-gray-600">
                        ...
                        dari <span class="font-semibold"><?= $total_rows ?></span> data jasa (50 data / halaman)
                    </p>
    ...
    <div class="modal-dialog" onclick="event.stopPropagation()" style="max-width: 500px; width: 90%;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-file-import"></i> Import Data Jasa</h5>
            ...
            <div class="modal-body" style="text-align: left;">
                <div class="note-box" style="margin-bottom: 15px; padding: 14px 16px;">
                    ...
                    <ul>
                        ...
                        <li>Kolom kode, nama, harga wajib terisi dengan benar.</li>
...
<script>
    ...
    function submitBulk(action) {
        ...
        let confirmMsg = '';
        if (action === 'nonaktif') {
            confirmMsg = '⚠️ Nonaktifkan ' + checked.length + ' data jasa terpilih?';
        } else if (action === 'aktifkan') {
            confirmMsg = '✅ Aktifkan kembali ' + checked.length + ' data jasa terpilih?\n\n(Jasa yang kodenya bentrok dengan jasa lain yang masih Aktif akan otomatis dilewati.)';
        } else if (action === 'hapus') {
            confirmMsg = '🗑️ HAPUS PERMANEN ' + checked.length + ' data jasa terpilih?\n\n⚠️ Jasa yang sudah pernah dipakai di permintaan perbaikan / riwayat otomatis akan dilewati (hanya bisa dinonaktifkan).\nData lainnya TIDAK BISA dikembalikan!';
        }
