...
if ($filter_status != '') {
    if ($filter_status == 'Menunggu_Pengawas') {
        $where = "WHERE p.status = 'Minta_Persetujuan_Pengawas' AND p.persetujuan_pengawas = 'Menunggu' AND k.bidang = 'ANGKUTAN DALAM'";
        if ($search != '') {
            $where .= " AND (k.nopol LIKE '%$search%' OR k.jenis_kendaraan LIKE '%$search%' OR k.bidang LIKE '%$search%' OR p.nomor_pengajuan LIKE '%$search%')";
        }
    } else {
        $where = "WHERE p.status = '$filter_status' AND k.bidang = 'ANGKUTAN DALAM'";
        if ($search != '') {
            $where .= " AND (k.nopol LIKE '%$search%' OR k.jenis_kendaraan LIKE '%$search%' OR k.bidang LIKE '%$search%' OR p.nomor_pengajuan LIKE '%$search%')";
        }
    }
...
$count_disetujui_karu = mysqli_num_rows(mysqli_query($connection, "
    SELECT p.* FROM permintaan_perbaikan p
    INNER JOIN kendaraan k ON p.id_kendaraan = k.id_kendaraan
    WHERE p.status='Disetujui_KARU_QC' AND k.bidang = 'ANGKUTAN DALAM'
"));

$count_menunggu_pengawas = mysqli_num_rows(mysqli_query($connection, "
    SELECT p.* FROM permintaan_perbaikan p
    INNER JOIN kendaraan k ON p.id_kendaraan = k.id_kendaraan
    WHERE p.status='Minta_Persetujuan_Pengawas' AND p.persetujuan_pengawas='Menunggu' AND k.bidang = 'ANGKUTAN DALAM'
"));
