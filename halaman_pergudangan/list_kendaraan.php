...
$where = "WHERE p.status IN ('Diajukan', 'Diperiksa_SA', 'Dikembalikan_sa') AND k.bidang = 'pergudangan'";
...
$count_diajukan = mysqli_num_rows(mysqli_query($connection, "
    ...
    WHERE p.status='Diajukan' AND k.bidang = 'pergudangan'
"));

$count_diperiksa = mysqli_num_rows(mysqli_query($connection, "
    SELECT p.* FROM permintaan_perbaikan p
    INNER JOIN kendaraan k ON p.id_kendaraan = k.id_kendaraan
    WHERE p.status='Diperiksa_SA' AND k.bidang = 'pergudangan'
"));

$count_belum_kirim = mysqli_num_rows(mysqli_query($connection, "
    SELECT p.* FROM permintaan_perbaikan p
    INNER JOIN kendaraan k ON p.id_kendaraan = k.id_kendaraan
    WHERE p.status='Dikembalikan_sa' AND p.persetujuan_pengawas IS NULL AND k.bidang = 'pergudangan'
"));

$count_menunggu = mysqli_num_rows(mysqli_query($connection, "
    SELECT p.* FROM permintaan_perbaikan p
    INNER JOIN kendaraan k ON p.id_kendaraan = k.id_kendaraan
    WHERE p.status='Dikembalikan_sa' AND p.persetujuan_pengawas='Menunggu' AND k.bidang = 'pergudangan'
"));

$count_ditolak = mysqli_num_rows(mysqli_query($connection, "
    SELECT p.* FROM permintaan_perbaikan p
    INNER JOIN kendaraan k ON p.id_kendaraan = k.id_kendaraan
    WHERE p.status='Dikembalikan_sa' AND p.persetujuan_pengawas='Ditolak' AND k.bidang = 'pergudangan'
"));

$count_disetujui_pengawas = mysqli_num_rows(mysqli_query($connection, "
    SELECT p.* FROM permintaan_perbaikan p
    INNER JOIN kendaraan k ON p.id_kendaraan = k.id_kendaraan
    WHERE p.status='Dikembalikan_sa' AND p.persetujuan_pengawas='Disetujui' AND k.bidang = 'pergudangan'
"));
